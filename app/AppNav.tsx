...
export default function AppNav(){
 ...
 const links=[
   {href:"/tickets",label:"Yêu cầu hỗ trợ",icon:"🎫"},
   {href:"/knowledge-base",label:"Kho kiến thức",icon:"📚"},
   ...(u.role==="admin"
     ? [{href:"/admin/users",label:"Người dùng",icon:"👥"},{href:"/admin/sla",label:"Thời hạn xử lý",icon:"⏱"}]
     : [])
 ];
     ...
     <div className="nav-inner">
       ...
       <div className="nav-user">
         <div className="user-profile">
           <span className="user-avatar">{u.name?.trim()?.charAt(0)?.toUpperCase()||"U"}</span>
   ...
   <div className="nav-menu">
     <div className="nav-menu-inner">
       <nav aria-label="Điều hướng chính">
         {links.map(x=><a key={x.href} className={path.startsWith(x.href)?"active":""} href={x.href}>
           <span className="nav-icon" aria-hidden="true">{x.icon}</span>
           <span>{x.label}</span>
         </a>)}
       </nav>
       <a className="nav-home-action" href="/tickets/new">＋ Tạo yêu cầu hỗ trợ</a>
     </div>
   </div>
 </header>;
}
