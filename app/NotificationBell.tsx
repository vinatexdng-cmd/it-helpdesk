...
export default function NotificationBell(){
 ...
 return <div className="notification-center" ref={ref}><button type="button" className="notification-bell" aria-label="Thông báo" onClick={()=>setOpen(v=>!v)}><span aria-hidden="true">🔔</span>{unread>0&&<span className="notification-count">{unread>99?"99+":unread}</span>}</button>{open&&<div className="notification-dropdown"><div className="notification-head"><div><strong>Thông báo</strong><small>{unread?`${unread} thông báo chưa đọc`:"Không có thông báo mới"}</small></div>{unread>0&&<button type="button" onClick={readAll}>Đánh dấu đã đọc</button>}</div><div className="notification-list">{items.length===0?<div className="notification-empty">Chưa có thông báo.</div>:items.slice(0,10).map(n=><button type="button" key={n.id} className={n.is_read?"notification-item":"notification-item unread"} onClick={()=>read(n)}><span className="notification-dot"/><span className="notification-body"><strong>{n.title}</strong><span>{n.message}</span><small>{time(n.created_at)}</small></span></button>)}</div></div>}</div>
}
