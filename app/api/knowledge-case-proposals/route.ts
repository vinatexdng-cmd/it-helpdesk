...
export async function PUT(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (!result.rowCount) { await client.query("ROLLBACK"); return NextResponse.json({ error: "Không tìm thấy Case hoặc Case đã được xuất bản" }, { status: 404 }); }
      const proposal = result.rows[0];
      await client.query(`INSERT INTO audit_logs(ticket_id,actor,action,details) VALUES($1,$2,$3,$4::jsonb)`,[proposal.ticket_id,user.email,status==="Approved"?"KNOWLEDGE_CASE_APPROVED":status==="Rejected"?"KNOWLEDGE_CASE_REJECTED":"KNOWLEDGE_CASE_UPDATED",JSON.stringify({proposal_id:proposal.id,ticket_no:proposal.ticket_no,status,review_note:body.review_note||""})]);

      if ((status === "Approved" || status === "Rejected") && proposal.generated_by && proposal.generated_by.toLowerCase() !== user.email.toLowerCase()) {
