...
export async function POST(req:NextRequest){
  try{
    ...
    const response=await fetch(baseUrl+"/chat/completions",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+apiKey},body:JSON.stringify({model,messages:[{role:"system",content:"Bạn là trợ lý CNTT nội bộ của Vinatex Đà Nẵng."},{role:"user",content:prompt}],temperature:0.1,max_completion_tokens:1600})});
