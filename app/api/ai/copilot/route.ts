...
export async function POST(req:NextRequest){
  try{
    ...
    const apiKey=process.env.AI_API_KEY,baseUrl=(process.env.AI_BASE_URL||"https://api.openai.com/v1").replace(/\/$/,""),model=process.env.AI_MODEL||"gpt-4o-mini";if(!apiKey)return NextResponse.json({error:"AI_API_KEY chưa được cấu hình trên server"},{status:503});
    ...
    const prompt=`Bạn là Vinatex IT Helpdesk Copilot, trợ lý CNTT nội bộ dựa trên Kho kiến thức.\n- Chỉ dùng SOURCE; không bịa.\n- Ưu tiên triệu chứng và phạm vi ảnh hưởng gần nhất.\n- Phân biệt lỗi một máy với lỗi diện rộng.\n- Nếu nguồn không đủ, nói rõ và hỏi tối đa 3 thông tin chẩn đoán.\n- Trả lời: Nhận định ngắn → Kiểm tra nhanh → Các bước xử lý → Cách xác nhận → Khi nào cần tạo yêu cầu/chuyển IT.\n- Cuối câu trả lời ghi Nguồn tham khảo: SOURCE X,... chỉ với nguồn đã dùng.\n- Dùng tiếng Việt rõ ràng.\n\nLỊCH SỬ:\n${conversation||"(chưa có)"}\n\nCÂU HỎI:\n${question}\n\nSOURCE:\n${context||"Không tìm thấy SOURCE phù hợp."}`;
    const response=await fetch(baseUrl+"/chat/completions",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+apiKey},body:JSON.stringify({model,messages:[{role:"system",content:"Bạn là trợ lý CNTT nội bộ của Vinatex Đà Nẵng."},{role:"user",content:prompt}],temperature:0.1,max_tokens:1600})});
    const data=await response.json();if(!response.ok)return NextResponse.json({error:data?.error?.message||`Nhà cung cấp AI trả về HTTP ${response.status}`},{status:502});const answer=data?.choices?.[0]?.message?.content;if(!answer)return NextResponse.json({error:"AI không trả về nội dung"},{status:502});
