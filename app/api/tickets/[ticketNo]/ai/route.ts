...
export async function POST(req:NextRequest){
 try{
  ...
  const p=getPool(), t=await p.query("SELECT * FROM tickets WHERE ticket_no=$1",[ticketNo]);
  if(!t.rowCount)return NextResponse.json({error:"Ticket not found"},{status:404});
  const ticket=t.rows[0], query=[ticket.title,ticket.description,ticket.category,ticket.asset,ticket.unit].filter(Boolean).join(" ");
