...
export async function POST(req:NextRequest){
 try{
  ...
  const apiKey=process.env.AI_API_KEY;if(!apiKey)return NextResponse.json({error:"AI chưa được cấu hình. Hãy khai báo AI_API_KEY trên Vercel."},{status:503});
  const p=getPool();
