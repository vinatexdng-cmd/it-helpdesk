...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ ticketNo: string }> }) {
  try {
    ...
    const r = await p.query("UPDATE tickets SET status=$1,assignee=$2,resolution=$3,priority=$4,updated_at=NOW() WHERE ticket_no=$5 RETURNING *", [status, assignee || null, resolution || null, priority, ticketNo]);
    const changes: Record<string, { from: unknown; to: unknown }> = {};
    for (const k of ["status","assignee","resolution","priority"]) if (old[k] !== r.rows[0][k]) changes[k] = { from: old[k], to: r.rows[0][k] };
    if (Object.keys(changes).length) await p.query("INSERT INTO audit_logs(ticket_id,actor,action,details) VALUES($1,$2,$3,$4)", [r.rows[0].id, u.email, "Ticket updated", JSON.stringify(changes)]);
