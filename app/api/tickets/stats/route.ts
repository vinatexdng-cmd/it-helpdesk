...
p.query("SELECT COALESCE(NULLIF(unit,''),'Chưa xác định') AS unit,COUNT(*)::int AS count FROM tickets"+where+" GROUP BY COALESCE(NULLIF(unit,''),'Chưa xác định') ORDER BY count DESC LIMIT 8",args),
p.query("SELECT COALESCE(NULLIF(category,''),'Other') AS category,COUNT(*)::int AS count FROM tickets"+where+" GROUP BY COALESCE(NULLIF(category,''),'Other') ORDER BY count DESC",args),
p.query("SELECT to_char(date_trunc('day',created_at),'YYYY-MM-DD') AS ticket_day,COUNT(*)::int AS count FROM tickets"+where+and+" created_at>=NOW()-INTERVAL '13 days' GROUP BY date_trunc('day',created_at) ORDER BY date_trunc('day',created_at)",args),
p.query("SELECT ticket_no,title,status,priority,unit,sla_due_at FROM tickets"+where+and+" sla_due_at IS NOT NULL AND status NOT IN ('Resolved','Closed') ORDER BY sla_due_at ASC LIMIT 6",args)]);return NextResponse.json({total:total.rows[0]?.count??0,statuses:statuses.rows,priorities:priorities.rows,overdue:overdue.rows[0]?.count??0,recent:recent.rows,units:units.rows,categories:categories.rows,trend:trend.rows.map(r=>({day:r.ticket_day,count:r.count})),dueSoon:dueSoon.rows})}catch(e){console.error("ticket stats error",e);return NextResponse.json({error:e instanceof Error?e.message:"Database error"},{status:500})}}
